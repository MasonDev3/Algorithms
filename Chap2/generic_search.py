...
class PriorityQueue(Generic[T]):
    ...
    @property
    def empty(self) -> bool:
        return not self._container  # not is true for empty
...
def astar(initial: T, goal_test: Callable[[T], bool],
    ...
    while not frontier.empty:
        current_node: Node[T] = frontier.pop()
        current_state: T = current_node.state
        # if we found our goal, we're done
        if goal_test(current_state):
            return current_node
        # check where we can go next and haven't explored
        for child in successors(current_state):
            new_cost: float = current_node.cost + 1  # 1 assumes a grid,
                                                     # need a cost function for more sophisticated apps
